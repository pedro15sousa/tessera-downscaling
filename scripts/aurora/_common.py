"""Helpers shared by the Aurora scripts (``generate_aurora_forecasts.py`` and
``extract_aurora_latents.py``): constants and variable mappings, the
valid-time / init-time schedule, staging paths and resume markers, the region
crop resolved against the dataset reference grid, the dry-run accounting, and
the heavy lifting (model load, ``Batch`` assembly, per-region frame writer).

Everything up to ``dry_run_report`` needs only numpy/pandas; the loaders and
writers import torch / aurora / xarray lazily so ``--dry-run`` stays light.
The generator's behaviour is unchanged by living here: both scripts import
this module (``from _common import ...``, the scripts/ convention), so the
latent extractor cannot drift from the forecast generator.
"""
...
# --------------------------------------------------------------------------- #
# Device selection and the CLI options shared by the two scripts.
# --------------------------------------------------------------------------- #


def resolve_device(requested: str | None, allow_cpu: bool = False):
    """``torch.device`` for a run: ``--device`` if given, else the first available
    accelerator (CUDA, then Intel XPU), else CPU -- which is refused unless
    ``allow_cpu`` because the global model exhausts host RAM."""
    import torch
    ...
    if device.type == "cpu" and not allow_cpu:
        ...
        raise SystemExit(
            ...
            "driver supports (or lacks the XPU backend). Install a torch build matching the\n"
            "node (mirror your working .venv), or pass --allow-cpu to override (debug only)."
        )
    return device


def add_common_arguments(p: argparse.ArgumentParser) -> argparse.ArgumentParser:
    """The options both Aurora scripts take, with the generator's defaults."""
    p.add_argument(
        "--global-metadata",
        required=True,
    ...
    p.add_argument("--device", default=None, help="e.g. cuda, xpu, cpu (default: auto)")
    p.add_argument(
        "--allow-cpu",
        action="store_true",
