...
Usage (from repo root):
    .venv/bin/python projects/tessera_downscaling/scripts/preprocessing/concat_station_vectors.py \
        --inputs  .../station_latents_lat16_grad0.5.npy .../extra_descriptors.npy \
        --output  .../station_latents_lat16_grad0.5_plus_extradesc.npy
"""
...
def main() -> int:
    ...
    if len(n_rows) != 1:
        raise ValueError(
            f"Row-count mismatch across inputs: { {p.name: a.shape for p, a in zip(args.inputs, arrays)} } "
