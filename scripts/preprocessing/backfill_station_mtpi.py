...
pipeline. It reuses the exact same join logic the preprocessors use
(:func:`helpers.lookup_station_mtpi`), so a backfilled dataset is identical to
one built fresh with ``--mtpi-csv``.
...
Inputs:
    --mtpi-csv    station_id,mtpi lookup from
...
Usage (from repo root):
    uv run --project projects/tessera_downscaling python \
        projects/tessera_downscaling/scripts/preprocessing/backfill_station_mtpi.py \
        --dataset-dir <.../dataset_timestamp_global> \
        --mtpi-csv    <.../station_mtpi.csv>
"""
...
from helpers import lookup_station_mtpi  # noqa: E402
