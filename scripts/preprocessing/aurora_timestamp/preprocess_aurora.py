...
Key points, all handled here:
  ...
  * Grid: Stage 1 already cropped each frame to the region grid (with the
    ...
    then stack.
  * Store-only regions: a region with no scaffolding in the global dataset (e.g.
    ``uk``, whose forecasts we stage for a later project) is skipped for dataset
    building with a warning -- its staging is left in place.
...
Usage:
    python preprocess_aurora.py \
        --global-dataset   /path/.tmp_output/dataset_timestamp_global \
        --aurora-staging-root /path/.tmp_output/_staging/aurora \
        --output-root      /path/.tmp_output \
        --leads 6 24 72 --split all \
...
# helpers.py lives one level up (scripts/preprocessing/); reuse its crop +
# channel-name machinery rather than reinventing it.
...
def aggregate_aurora_snapshot(source_root: Path, date_str: str, hour: int) -> np.ndarray | None:
    ...
    """
    import xarray as xr
    from helpers import ATMOS_VARS, PRESSURE_LEVELS, SURFACE_VARS
...
# --------------------------------------------------------------------------- #
# Normalisation-stats: drop the precip entry from the global ERA5 stats.
# --------------------------------------------------------------------------- #
...
def build_region(
    ...
    if workers <= 1:
        ...
        for i, ts in enumerate(split_times, 1):
            w, s = _build_one_snapshot(ts); written += w; skipped += s; _tick(i)
    else:
...
def build_lead(
    ...
    from helpers import era5_snapshot_channel_names
    ...
    for stats_name in ("normalisation_stats_global.npz", "normalisation_stats_no_static_global.npz"):
        src = global_dataset / stats_name
        if src.exists():
            drop_precip_from_stats(src, out_dataset / stats_name, precip_idx)
...
def parse_args():
    ...
    p.add_argument("--global-dataset", type=Path, required=True, help="dataset_timestamp_global directory")
    p.add_argument("--aurora-staging-root", type=Path, required=True,
                   help="_staging/aurora (contains lead{L}h/<region>/processed)")
    p.add_argument("--output-root", type=Path, required=True, help="Where dataset_timestamp_aurora_lead{L}h dirs are written")
    ...
    p.add_argument("--regions", type=str, nargs="+", default=["europe", "east_asia"],
                   help="Dataset regions to build (store-only regions like 'uk' are skipped automatically).")
