...
def nan_highpass(a, sigma):
    ...
    low = np.where(
        den > 1e-6, gaussian_filter(af * m, sigma) / np.maximum(den, 1e-9), np.nan
    )
    return a - low
...
def metrics(var, ts):
    ...
    row = dict(
        var=var,
        ts=ts,
        elevation=("DEM" if dem else "proxy"),
        finestd_era5=se,
        finestd_base=sb,
        finestd_tess=st,
        ...
        diff_p5=float(np.nanpercentile(diff, 5)),
        diff_p95=float(np.nanpercentile(diff, 95)),
    ...
    if sp.exists():
        ...
        if m.any():
            ...
            row.update(
                n_stn=int(m.sum()),
                mae_era5=mae("era5_err"),
                mae_base=mae("base_err"),
                mae_tess=mae("tess_err"),
                rmse_era5=rmse(rk("era5_err")),
                rmse_base=rmse(rk("base_err")),
...
def per_snapshot_table(row):
    ...
    cell = [
        [
            "ERA5-interp (no model)",
            f"{row['mae_era5']:.2f}",
            f"{row['rmse_era5']:.2f}",
            f"{row['finestd_era5']:.3f}",
            f"{row['finestd_era5'] / row['finestd_base']:.2f}",
            "—",
        ],
        [
            "ConvCNP baseline (no TESSERA)",
            f"{row['mae_base']:.2f}",
            f"{row['rmse_base']:.2f}",
            f"{row['finestd_base']:.3f}",
            "1.00",
            f"{row['rug_r_base']:+.2f}",
        ],
        [
            "ConvCNP + lat16 (TESSERA)",
            f"{row['mae_tess']:.2f}",
            f"{row['rmse_tess']:.2f}",
            f"{row['finestd_tess']:.3f}",
            f"{row['tess_x_base']:.2f}",
            f"{row['rug_r_tess']:+.2f}",
        ],
    ]
    ...
    ax.axis("off")
    tbl = ax.table(
        cellText=[r[1:] for r in cell],
        rowLabels=[r[0] for r in cell],
        colLabels=cols,
        loc="center",
        cellLoc="center",
        rowLoc="center",
...
def region_rollup(rows):
    ...
    for j in range(len(cols)):
        tbl[0, j].set_text_props(weight="bold")
    ax.set_title(
        f"{R.name.capitalize()} — snapshot comparison (MAE in variable units; "
        f"'fine x base' = TESSERA/baseline fine-scale std)",
        fontsize=11,
    )
    png_p = R.out_dir / f"{R.name}_summary.png"
    fig.savefig(png_p, dpi=160, bbox_inches="tight")
    plt.close(fig)
    return csv_p, png_p
...
def main():
    ...
    for sub in subs:
        ...
        rows.append(row)
        print(
            f"  {sub.name}: MAE base/tess={row['mae_base']:.2f}/{row['mae_tess']:.2f}  "
