...
def ens_point(reg, stem, var, suffix=""):
    ...
    for s in SEEDS:
        ...
        if not f.exists():
            return None
        d = np.load(f)
        pts.append(point(d, var)); tgt = d[f"{var}_targets"].astype(np.float64)
...
def main():
    ...
    print("Full test set, MAIN-RESULTS models (t2m gaussian+mTPI, wind truncnormal+mTPI),")
    print("3-seed, MAE@median point estimate (stable truncated median for wind):\n")
    hdr = ("region", "var", "n", "MAEb", "MAEt", "dMAE%", "CRPSb", "CRPSt", "dCRPS%",
           "R2", "hit%", "rho", "null")
    print("".join(f"{h:>{w}}" for h, w in zip(
        hdr, [16, 5, 9, 7, 7, 6, 7, 7, 7, 6, 6, 6, 7])))
    pool = {}
    for reg in REGIONS:
        for var in ["t2m", "wind"]:
            ...
            if rb is None or rt is None:
                print(f"{reg:>16}{var:>5}  (missing)"); continue
            yb, tb = rb; yt, tt = rt
            if not (len(tb) == len(tt) and np.allclose(tb, tt)):
                print(f"{reg:>16}{var:>5}  [misaligned] nb={len(tb)} nt={len(tt)}"); continue
            y = tb; e = y - yb; d = yt - yb; g = np.abs(e) - np.abs(e - d)
            r = pearsonr(d, e).statistic; hit = np.mean(np.sign(d) == np.sign(e))
            idx = rng.choice(len(e), size=min(80000, len(e)), replace=False)
            es, ds = e[idx], d[idx]
            rho = spearmanr(np.abs(ds), g[idx]).statistic
            null = np.mean([spearmanr(np.abs(f := rng.normal(0, ds.std(), len(ds))),
                                      np.abs(es) - np.abs(es - f)).statistic
                            for _ in range(20)])
            ...
            print(f"{reg:>16}{var:>5}{len(e):>9}{maeb:>7.2f}{maet:>7.2f}{dmae:>6.1f}"
    ...
    for var in ["t2m", "wind"]:
        ...
        r = pearsonr(D, E).statistic; hit = np.mean(np.sign(D) == np.sign(E))
        print(f"  {var}: N={len(E):,}  R2={r**2:.2f}  pearson={r:+.2f}  hit={100*hit:.1f}%")
