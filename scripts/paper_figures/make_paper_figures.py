...
MAP_PANEL_TITLES = ["ERA5 bilinear interpolation", "ConvCNP (topography-only)",
...
def _map_inputs(reg, var, ts):
    d = np.load(MAPS_OUT / reg / f"{var}_{ts}" / f"{reg}_{var}_{ts}_dem.npz",
                allow_pickle=True)
    arrays = [np.asarray(d["era5_interp"]), np.asarray(d["convcnp_baseline"]),
              np.asarray(d["tessera_concat"])]
    lons, lats = np.asarray(d["lons"]), np.asarray(d["lats"])
    dem = np.load(DATA / "processed" / "dense" / reg
                  / f"{reg}_0.05deg_dem.npy").reshape(len(lats), len(lons))
...
def _drape(ax, field, cmap, norm, ls, elev_filled, dx, dy, extent, proj):
    rgb = plt.get_cmap(cmap)(norm(np.where(np.isfinite(field), field,
                                           0.0)))[..., :3]
...
def fig03(out: Path) -> None:
    ...
    for reg, var, ts in PAPER_MAPS:
        ...
        fig, axes = plt.subplots(1, 3, figsize=(TEXTWIDTH, 3.4),
                                 subplot_kw={"projection": proj})
        im = None
        for ax, arr, t in zip(axes, arrays, MAP_PANEL_TITLES):
...
def fig04(out: Path) -> None:
    ...
    for reg, var, ts in PAPER_MAPS:
        ...
        step = min([s for s in (100, 200, 250, 500, 1000)
                    if s * N_CONTOURS >= e_top] or [1000])
        ...
        _drape(axes[0], dem_land, "terrain", e_norm, ls, elev_filled, dx, dy,
               extent, proj)
        ...
        panels = [(MAP_DIFF_TITLE, raw),
                  (MAP_DIFF_TITLE + "\n"
                   + rf"(mean {mean:+.2f} {unit} removed)", anom)]
        for ax, (title, fld) in zip(axes[1:], panels):
            _drape(ax, fld, "RdBu_r", d_norm, ls, elev_filled, dx, dy,
                   extent, proj)
            if len(levels):
                ax.contour(lons, lats, dem_land, levels=levels, colors="k",
                           linewidths=0.2, alpha=0.45, transform=proj)
            ax.coastlines(resolution="10m", linewidth=0.4, color="k")
            ax.set_extent(extent, crs=proj)
            ax.set_title(title, fontsize=7)
            _map_cbar(fig, ScalarMappable(norm=d_norm, cmap="RdBu_r"), ax,
                      rf"$\Delta$ {unit}")
        axes[1].text(0.5, -0.06, f"contours every {step:.0f} m",
...
def _agg(region: str, run_name: str, var: str):
    recs = [r for s in SEEDS
            if (r := _read(DATA / f"training_runs_{region}" / f"{run_name}_seed{s}",
                           var)) is not None]
    if not recs:
        return None
...
FIG2_EXPECTED = {
    "Europe":    dict(t2m=6.332653, wind=6.240472, n_t2m=5825, n_wind=3567,
                      rho_t2m=321.972445, rho_wind=197.163212),
    "USA":       dict(t2m=7.934198, wind=6.252978, n_t2m=2403, n_wind=2310,
                      rho_t2m=159.196618, rho_wind=153.035450),
    "E. Asia":   dict(t2m=19.068599, wind=5.225167, n_t2m=585, n_wind=585,
                      rho_t2m=47.335539, rho_wind=47.335539),
    "S. Africa": dict(t2m=9.231471, wind=5.599284, n_t2m=223, n_wind=223,
                      rho_t2m=49.925590, rho_wind=49.925590),
    "Australia": dict(t2m=14.980941, wind=7.739540, n_t2m=140, n_wind=134,
                      rho_t2m=8.930443, rho_wind=8.547709),
}
...
def _fig2_rows():
    ...
    for region, (disp, bbox_key) in FIG2_REGIONS.items():
        ...
        for var, _lab, _col, (base_run, tess_run) in FIG2_SERIES:
            ...
            rec[f"uplift_{var}"] = ((b["crps"] - t["crps"]) / b["crps"] * 100
            ...
            for run in (TRAIN_COUNT_RUN[var], base_run):
                for s in SEEDS:
                    r = _read(DATA / f"training_runs_{region}"
                              / f"{run}_seed{s}" / "eval_train_stations", var)
    ...
    for rec in rows:
        ...
        for var in ("t2m", "wind"):
            for field, want in ((f"uplift_{var}", exp[var]),
                                (f"n_{var}", exp[f"n_{var}"]),
                                (f"rho_{var}", exp[f"rho_{var}"])):
                got = rec[field]
                if not np.isfinite(got):
                    print(f"  [warn] fig02 {rec['label']}/{field}: no local "
                          f"data, using stored value {want:.3f}")
                    rec[field] = want
                elif abs(got - want) > max(0.05, 0.002 * abs(want)):
                    print(f"  [warn] fig02 {rec['label']}/{field}: recomputed "
...
def fig02(out: Path) -> None:
    ...
    for x, r in zip(xs, rows):
        for i, (var, *_rest) in enumerate(FIG2_SERIES):
            ...
            _below(x + off, RHO_DY, _fmt_rho(r[f"rho_{var}"]),
...
def fig02ext(out: Path) -> None:
    ...
    EXTRA_RUN = {"t2m": "t2m_snap_bilinear_baseline_mtpi_extradesc_wd",
                 "wind": "wind_truncnormal_snap_bilinear_baseline_mtpi_extradesc_wd"}
    SHADE = {("t2m", "base"): "#d95f02", ("t2m", "extra"): "#fdae6b",
             ("wind", "base"): "#1b7837", ("wind", "extra"): "#7fbf7b"}

    rows = _fig2_rows()
    # add the vs-extradesc uplifts + the paper's appendix cross-check
    extra_gain = {"t2m": [], "wind": []}
    for rec, (region, _d) in zip(rows, FIG2_REGIONS.items()):
        for var, _lab, _col, (base_run, tess_run) in FIG2_SERIES:
            b = _agg(region, base_run, var)
            bx = _agg(region, EXTRA_RUN[var], var)
            t = _agg(region + TESS_SUFFIX, tess_run, var)
            rec[f"uplift_extra_{var}"] = ((bx["crps"] - t["crps"])
                                          / bx["crps"] * 100
                                          if bx and t else float("nan"))
            if b and bx:
    ...
    for var, want in (("t2m", 3.2), ("wind", 2.2)):
        ...
        print(f"  [{tag}] extradesc-vs-plain CRPS gain {var}: "
              f"{got:.2f}% (paper: {want}%)")

    xs = np.arange(len(rows))
    width, gap = 0.19, 0.015
    fig, ax = plt.subplots(figsize=(TEXTWIDTH, 3.3))
    bars_spec = [("t2m", "base", "uplift_t2m"),
    ...
    for x, r in zip(xs, rows):
        for var in ("t2m", "wind"):
            n = r[f"n_{var}"]
            _below(x + pair_off[var], N_DY, "—" if n != n else f"{int(n):,}",
                   fontsize=7, color="#333333")
            _below(x + pair_off[var], RHO_DY, _fmt_rho(r[f"rho_{var}"]),
                   fontsize=7, color="#666666")
        for dy in (N_DY, RHO_DY):
            _below(x, dy, "|", fontsize=7, color="#aaaaaa")
    for dy, txt in ((N_DY, "n"), (RHO_DY, "ρ")):
...
def fig09(out: Path) -> None:
    ...
    for reg, var, ts in PAPER_MAPS:
        ...
        ext = [float(lons[0]), float(lons[-1]),
               float(lats[-1]), float(lats[0])]
        inb = ((st["lon"] >= ext[0]) & (st["lon"] <= ext[1])
               & (st["lat"] >= ext[2]) & (st["lat"] <= ext[3]))
    ...
    for ax, (var, meta) in zip(axes, panels.items()):
        ...
        for x0, y0 in ((0, 0), (-lim, -lim)):
            ax.add_patch(plt.Rectangle((x0, y0), lim, lim, facecolor="0.92",
                                       edgecolor="none", zorder=0))
        ax.plot([-lim, lim], [-lim, lim], ls="--", lw=0.8, color="0.55",
                zorder=2)
        xs = np.array([-lim, lim])
        fit_line, = ax.plot(xs, beta * xs + icpt, color="k", lw=1.1, zorder=3,
                            label=f"fit ($\\hat\\beta$={beta:.2f}, "
                                  f"$R^2$={r2:.2f})")
        for reg in ("iberia", "norway"):
            dl, e = data[var][reg]
            ax.scatter(dl, e, s=8, alpha=0.75, lw=0, color=colours[reg],
                       zorder=4)
        ax.set_xlim(-lim, lim)
        ax.set_ylim(-lim, lim)
        ax.set_aspect("equal")
        ...
        u = meta["unit"]
        ax.set_xlabel("TESSERA increment "
                      f"$\\Delta = \\hat{{y}}_T - \\hat{{y}}_b$  ({u})")
        ax.set_ylabel(f"baseline error  $e = y - \\hat{{y}}_b$  ({u})")
        # per-panel legend: only the fit line (its stats differ per panel)
        ax.legend(handles=[fit_line], loc="upper left", frameon=True,
                  framealpha=1.0, facecolor="white", edgecolor="0.8",
                  fontsize=7, borderpad=0.3, handlelength=1.0,
                  handletextpad=0.5, borderaxespad=0.3)

    # shared entries once, below the panels
    from matplotlib.lines import Line2D
    shared = [Line2D([0], [0], ls="--", lw=0.8, color="0.55",
                     label="perfect ($\\Delta = e$)"),
              Line2D([0], [0], ls="none", marker="o", ms=4,
                     color=colours["iberia"], label="iberia"),
              Line2D([0], [0], ls="none", marker="o", ms=4,
                     color=colours["norway"], label="norway")]
    fig.legend(handles=shared, loc="outside lower center", ncol=3,
...
EVAL_SOURCES = {"eval_lead0h": "ERA5 (lead 0)", "eval_lead6h": "Lead +6h",
                "eval_lead24h": "Lead +24h", "eval_lead72h": "Lead +72h"}
...
def _xlead_frame():
        ...
        for (target, variant), (root, cfg) in XLEAD_CFG.items():
            for seed in SEEDS:
                for ek, lbl in EVAL_SOURCES.items():
                    ...
                    rows.append(dict(region=region, target=target,
                                     variant=variant, seed=seed, eval_label=lbl,
                                     rmse=rmse, mae=mae,
    ...
    if csv.exists():
        ...
        m = df.merge(ref, on=["region", "target", "variant", "seed", "eval_label"],
                     suffixes=("", "_ref"))
        for col in ("rmse", "crps"):
            bad = m[(m[col] - m[f"{col}_ref"]).abs() > 1e-6]
            if len(bad):
                print(f"  [warn] fig06/10: {len(bad)} {col} cells differ from "
...
def fig06(out: Path) -> None:
    ...
    piv = D.pivot_table(index=["region", "target", "seed", "eval_label"],
                        columns="variant", values=metric, observed=True)
    piv = piv.dropna(subset=["baseline", "concat"])
    up = ((piv["baseline"] - piv["concat"]) / piv["baseline"] * 100).reset_index()
    up = (up.groupby(["region", "target", "eval_label"], observed=True)[0]
            .agg(["mean", "std"]).reset_index())

    # black: concat seeds vs deterministic interp reference
    mdl = (D[D.variant == "concat"]
           .groupby(["region", "target", "eval_label"], observed=True)[metric]
           .agg(m="mean", s="std").reset_index())
    mdl["ref"] = [ref.get((r, t, l))
                  for r, t, l in zip(mdl.region, mdl.target, mdl.eval_label)]
    mdl["mean"] = (mdl["ref"] - mdl["m"]) / mdl["ref"] * 100
    mdl["std"] = mdl["s"] / mdl["ref"] * 100

    for (reg, tgt), want in FIG6_EXPECTED_BLUE.items():
        got = (up[(up.region == reg) & (up.target == tgt)]
               .set_index("eval_label")["mean"].reindex(EVAL_ORDER).values)
    ...
    for (reg, tgt), want in FIG6_EXPECTED_BLACK.items():
        got = (mdl[(mdl.region == reg) & (mdl.target == tgt)]
               .set_index("eval_label")["mean"].reindex(EVAL_ORDER).values)
        if not np.allclose(got, want, atol=0.01):
    ...
    LBL_BLACK = ("vs bilinear interp of the 0.25° context field "
                 "(+ fitted lapse rate for t2m)")
    fig, axes = plt.subplots(2, 2, figsize=(TEXTWIDTH, 4.3),
    ...
    for ri, target in enumerate(["t2m", "wind"]):
        for ci, region in enumerate(XLEAD_REGIONS):
            ...
            for frame, col, mk in ((up, BLUE, "o"), (mdl, BLACK, "D")):
                ...
                ax.errorbar(x, a["mean"].values, yerr=a["std"].values,
                            marker=mk, color=col, lw=1.2, ms=3.5, capsize=2)
            ...
            if ci == 0:
                ax.set_ylabel(f"{target}\n{metric.upper()} uplift (% better)",
                              fontweight="bold")
    handles = [Line2D([0], [0], color=BLUE, marker="o", lw=1.2, ms=3.5,
                      label=LBL_BLUE),
               Line2D([0], [0], color=BLACK, marker="D", lw=1.2, ms=3.5,
                      label=LBL_BLACK)]
    fig.legend(handles=handles, loc="outside lower center", ncol=2,
               frameon=False, fontsize=7)
...
def fig10(out: Path) -> None:
    metric = "crps"
    D = _xlead_frame()
    piv = (D.pivot_table(index=["region", "target", "variant", "seed"],
                         columns="eval_label", values=metric, observed=True)
    ...
    for ci, region in enumerate(XLEAD_REGIONS):
        ...
        for target in ("t2m", "wind"):
            for variant in ("baseline", "concat"):
                ...
                ax.plot(x, m, ls=REL_DASH[variant], marker=REL_MARKER[variant],
                        color=TARGET_COLORS[target], lw=1.2, ms=3.5)
                ax.fill_between(x, m - s, m + s, color=TARGET_COLORS[target],
                                alpha=0.12, lw=0)
        ax.axhline(0, color="k", lw=0.6)
        ax.axvline(0.5, color="k", ls=":", lw=0.6, alpha=0.5)
        ax.grid(True, alpha=0.25)
    ...
    handles += [Line2D([0], [0], color=TARGET_COLORS[t], lw=1.8, label=t)
                for t in ("t2m", "wind")]
    handles += [_hdr(r"$\bf{variant}$")]
    handles += [Line2D([0], [0], color="0.35", lw=1.2, ls=REL_DASH[v],
                       marker=REL_MARKER[v], ms=3.5, label=VLABEL[v])
    ...
    axes[0].legend(handles=handles, loc="upper left", fontsize=7,
                   ...
                   labelspacing=0.35, borderaxespad=0.4, borderpad=0.35)
...
SRX_SPACES = ["geographic", "elevation+mTPI", "terrain stats (7f)",
              "land cover (10f)", "extended surface (17f)",
              "ERA5-static", "TESSERA"]
SRX_NEW = ["terrain stats (7f)", "land cover (10f)", "extended surface (17f)"]
SR_COLOUR = {"geographic": "#7f7f7f", "elevation+mTPI": "#ff7f0e",
             "ERA5-static": "#9467bd", "TESSERA": "#1f77b4",
             "terrain stats (7f)": "#e377c2", "land cover (10f)": "#8c564b",
             "extended surface (17f)": "#c49a6c"}
SRX_TERRAIN = {"elev_mean", "elev_std", "elev_min", "elev_max",
               "slope", "dz_dn", "dz_de"}

# Stored notebook tables (§3c/§3g outputs) — cross-check targets.
FIG5_EXPECTED = {
    ("Europe", "t2m"): dict(zip(SR_SPACES, (0.014, 0.646, 0.089, 0.186))),
    ("Europe", "wind"): dict(zip(SR_SPACES, (0.083, 0.117, 0.165, 0.271))),
    ("United States", "t2m"): dict(zip(SR_SPACES, (-0.115, 0.003, -0.116, 0.012))),
    ("United States", "wind"): dict(zip(SR_SPACES, (0.254, 0.037, 0.154, 0.312))),
}
...
def _sr_elev_lut():
    if "elev" not in _SR_CACHE:
        s = _sr_stations_df()
        _SR_CACHE["elev"] = {sid: (float(e), float(de), float(m))
                             for sid, e, de, m in zip(s.station_id, s.elevation,
                                                      s.delta_elevation, s.mtpi)}
...
def _sr_era5_lut(region_key):
    ...
    if key not in _SR_CACHE:
        ...
        E = np.column_stack([
            RegularGridInterpolator((glat, glon), sf[c], method="linear",
                                    bounds_error=False, fill_value=None)(q)
            for c in range(sf.shape[0])]).astype(np.float32)
...
def _sr_latents(folder, var):
    ...
    if key not in _SR_CACHE:
        ...
        lut = None
        for s in SEEDS:
            cfgp = (DATA / f"training_runs_{folder}_tessera_1B-M_2017"
                    / f"{SR_TESS_STEM[var]}_seed{s}" / "config.json")
            if not cfgp.exists():
                continue
...
def _sr_era5_target(folder, var):
    ...
    for s in SEEDS:
        ...
        return {str(sids[i]): (float(lats[i]), float(lons[i]), sums[i] / cnts[i])
                for i in range(nst) if cnts[i] > 0}
    return {}
...
def _sr_extra_lut():
    ...
    if "extra" not in _SR_CACHE:
        ...
        ids = pd.read_csv(DATA / "processed" / "tessera_global"
                          / "station_list_filtered.csv")["station_id"].astype(str).values
        names = json.loads((DATA / "processed"
                            / "extra_descriptors_names.json").read_text())["columns"]
...
def _sr_probe(spaces):
    ...
    if key in _SR_CACHE:
        return _SR_CACHE[key]
    extra_lut, extra_names = (_sr_extra_lut() if set(spaces) & set(SRX_NEW)
                              else (None, []))
    ...
    for folder, (disp, rk) in SR_WELL_SAMPLED.items():
        for var in ("t2m", "wind"):
            ...
            feat = {"geographic": np.column_stack([lon * np.cos(mlat), lat]),
            ...
            row["n"] = len(shared)
            out[(disp, var)] = row
            print(f"  [probe] {disp}/{var}: n={len(shared)}  "
...
def _sr_check(table, expected, figname, atol=0.03):
    for (disp, var), exp in expected.items():
        ...
        if got is None:
            ...
            continue
        vals = exp.items() if isinstance(exp, dict) else zip(SRX_NEW, exp)
        for sp, want in vals:
            if abs(got[sp] - want) > atol:
                print(f"  [warn] {figname} {disp}/{var}/{sp}: recomputed "
...
def _sr_bars(ax, spaces, table, var, wrap=False, rotation=30, ha="right"):
    ...
    ax.bar(x, means, yerr=sds, color=[SR_COLOUR[s] for s in spaces],
           edgecolor="black", linewidth=0.5, zorder=2, capsize=2.5,
...
def _sr_tag(ax, tag):
    ax.text(0.02, 0.97, tag, transform=ax.transAxes, ha="left", va="top",
...
def fig05(out: Path) -> None:
    ...
    for ax, tag, var in zip(axes, "ab", ("t2m", "wind")):
...
def fig08(out: Path) -> None:
    ...
    for ax, tag, var in zip(axes, "ab", ("t2m", "wind")):
        ...
        means, sds = _sr_bars(ax, SRX_SPACES, table, var, wrap=True,
                              rotation=0, ha="center")
        lo = min(0.0, float((means - sds).min()))
        hi = float((means + sds).max())
        span = max(hi - lo, 1e-6)
        ax.set_ylim(lo - 0.05 * span, hi + 0.24 * span)
        ax.text((min(newix) + max(newix)) / 2, 0.99,
                "land surface (hand-crafted)",
                transform=ax.get_xaxis_transform(), ha="center", va="top",
                fontsize=7, color="0.35")
        for xi, m, sd in zip(np.arange(len(SRX_SPACES)), means, sds):
            ax.text(xi, m + sd + 0.02 * span, f"{m:+.2f}", ha="center",
...
_AH_SQRT = (lambda x: np.sign(x) * np.sqrt(np.abs(x)),
...
def _roll_runs():
    ...
    for folder, want_tess in ((ROLL_FOLDER, False), (ROLL_TESS, True)):
        ...
        for d in sorted(root.iterdir()):
            ...
            recs.append(dict(run_dir=d, arch=arch, family=family, var=var,
                             sweep=sweep, x=SWEEP_X_YEARS.get(sweep),
...
def _roll_station_arrays(run_dir, var):
    ...
    if f"{var}_station_mae" not in d.files:
        return None
    return (d["station_ids"].astype(str), d["station_lats"], d["station_lons"],
            d["subset_per_station"], d[f"{var}_station_count"].astype(float),
            d[f"{var}_station_mae"].astype(float))


def _roll_col_sel(ids, lat, lon, sub, cnt, sweep, colkey):
    has = cnt > 0
    in_nor = ((lat >= NB_LAT[0]) & (lat <= NB_LAT[1])
              & (lon >= NB_LON[0]) & (lon <= NB_LON[1]))
    dep_ids = {sid for sid, v in
               _roll_sched()["sweep_points"][sweep]["probe_active_from"].items()
...
def _roll_micro(val, cnt, sel):
    return (float((val[sel] * cnt[sel]).sum() / cnt[sel].sum())
...
def fig07(out: Path) -> None:
    import pandas as pd
    recs = _roll_runs()
    cols = [("probe_deployed", "Deployed Norwegian stations\n(in training)"),
    ...
    for r in recs:
        ...
        for colkey, _t in cols:
            ...
            rows.append(dict(var=r["var"], family=r["family"], x=r["x"],
                             seed=r["seed"], col=colkey,
    ...
    for var in ("t2m", "wind"):
        ...
        b = next((r for r in recs if r["var"] == var
                  and r["family"] == "baseline"), None)
        if e is None or b is None:
            continue
        d = np.load(Path(e["run_dir"]) / "test_station_errors.npz",
                    allow_pickle=True)
        rr = np.load(Path(b["run_dir"]) / "test_station_errors.npz",
                     allow_pickle=True)
        ids = d["station_ids"].astype(str)
        rids = rr["station_ids"].astype(str)
        sub_of = dict(zip(rids, rr["subset_per_station"].astype(str)))
        ...
        cnt = np.where([i in scored for i in ids],
                       d[f"{var}_station_count"].astype(float), 0.0)
        era[var] = (ids, d["station_lats"], d["station_lons"], sub, cnt,
                    d[f"{var}_station_mae"].astype(float))

    sched = _roll_sched()
    dep_of = {lbl: sum(1 for v in
                       sched["sweep_points"][lbl]["probe_active_from"].values()
                       if not str(v).startswith("9999")) for lbl in SWEEP_ORDER}
    dep_sweeps = ["r1mo", "r6mo", "r1y", "r2y", "r3y"]

    fig, axes = plt.subplots(2, 2, figsize=(TEXTWIDTH, 4.8), sharex="col",
                             sharey="row", squeeze=False, layout="constrained")
    fig.get_layout_engine().set(w_pad=0.01, h_pad=0.01, wspace=0.05,
                                hspace=0.06)
    for i, var in enumerate(("t2m", "wind")):
        for j, (colkey, title) in enumerate(cols):
            ...
            for family in ("tessera", "baseline"):
                a = (F[(F["var"] == var) & (F.family == family)
                       & (F.col == colkey)]
                     .groupby("x")["mae"].agg(["mean", "std"]).reset_index()
                     .sort_values("x").dropna(subset=["mean"]))
                ax.errorbar(a["x"], a["mean"], yerr=a["std"].fillna(0),
            ...
            if var in era:
                ...
                ex, ey = [], []
                for lbl in SWEEP_ORDER:
                    esel = _roll_col_sel(eids, elat, elon, esub, ecnt, lbl,
                                         colkey)
            ...
            if colkey == "probe_deployed" and i == 0:
                ...
                xs = [SWEEP_X_YEARS[l] for l in dep_sweeps]
                top.set_xticks(xs)
                top.set_xticklabels([str(dep_of[l]) for l in dep_sweeps],
                                    fontsize=_AH_FS["toptick"])
                top.set_xlim(*xlim)
                top.tick_params(axis="x", length=2.5, pad=1.5)
                top.set_xlabel("# probe stations deployed",
                               fontsize=_AH_FS["toplabel"], labelpad=5)
    ...
    fig.legend([hl[l] for l in order], order, loc="outside lower center",
               ncol=len(order), frameon=False, fontsize=_AH_FS["legend"],
...
def _nw_probe_report(extra: bool = False):
    ...
    for name, X in spaces.items():
        ...
        rep[name] = dict(reach=100 * float(np.mean(dq <= r95)),
                         auc=_nw_auc(Xa, ya))
        if name in NW_EXPECTED:
            want_r, want_a = NW_EXPECTED[name]
            if (abs(rep[name]["reach"] - want_r) > 4
                    or abs(rep[name]["auc"] - want_a) > 0.03):
                print(f"  [warn] fig11/12 {NW_LABEL[name]}: reach "
                      f"{rep[name]['reach']:.0f}% auc {rep[name]['auc']:.2f} "
                      f"vs docstring {want_r}%/{want_a}")
        else:
            print(f"  [info] {NW_LABEL[name]}: reach {rep[name]['reach']:.0f}%"
...
def _nw_cluster_panel(a, name, spaces, groups, norway, sub_mask, rep):
    ...
    if name.startswith("geographic"):
        ...
        py = PCA(n_components=1).fit(
            resid[groups["rest_train"]]).transform(resid)[:, 0]
        P = np.column_stack([px, py])
        xl, yl = "Norway-vs-rest discriminant", "residual PC1"
    a.scatter(P[sub_mask, 0], P[sub_mask, 1], s=2, c="#cccccc",
              rasterized=True)
    a.scatter(P[groups["nonnorway_test"], 0], P[groups["nonnorway_test"], 1],
              s=3, c="#2ca02c", alpha=0.6, lw=0, rasterized=True)
    a.scatter(P[norway, 0], P[norway, 1], s=3, c="#d62728", alpha=0.6,
              lw=0, rasterized=True)
    r = rep[name]
    a.set_title(f"{name.replace(chr(10), ' ')}\nreachable "
                f"{r['reach']:.0f}%,  AUC {r['auc']:.2f}", fontsize=8)
...
def _nw_legend_handles():
    from matplotlib.lines import Line2D
    return [Line2D([0], [0], ls="none", marker="o", ms=4, color=c, label=l)
            for c, l in (("#cccccc", "rest-train"),
...
def _nw_masks(groups, st):
    ...
    sub = rng.choice(np.where(groups["rest_train"])[0],
...
def fig11(out: Path) -> None:
    ...
    for a, name in zip(axes.flat, NW_SPACE_ORDER):
        _nw_cluster_panel(a, name, spaces, groups, norway, sub_mask, rep)
    fig.legend(handles=_nw_legend_handles(), loc="outside lower center",
...
def fig12(out: Path, extra: bool = False) -> None:
    ...
    for name, X in spaces.items():
        Xs = StandardScaler().fit(X[groups["rest_train"]]).transform(X)
        d_loo = NearestNeighbors(n_neighbors=2).fit(
            Xs[groups["rest_train"]]).kneighbors(Xs[groups["rest_train"]])[0][:, 1]
    ...
    def reach_auc_rows(deployed):
        ...
        for name, (Xs, r95) in prep.items():
            ...
            Xa = np.vstack([qq, ref])
            ya = np.r_[np.ones(len(qq)), np.zeros(len(ref))]
            rows.append(dict(x=n_dep, space=name,
    ...
    for lbl in ("r1mo", "r3mo", "r6mo", "r1y", "r2y", "r3y"):
        ...
        dep_ids = {sid for sid, v in sp["probe_active_from"].items()
                   if not str(v).startswith("9999")}
    ...
    for name in order:
        ...
        for a, ycol in zip(ax, ("reach", "auc")):
            a.plot(s["x"], s[ycol], marker="o", ms=3, color=NW_COLOUR[name],
                   lw=1.2, label=NW_LABEL[name])
    ax[0].set_ylim(-5, 108)
    ax[0].set_ylabel("% of out-of-training Norwegian\nstations with an "
    ...
    ax[0].text(15, -3, "cold start", fontsize=7, color="#666666",
               va="center", ha="left")
    save(fig, out, "fig12_norway_reach_horizon"
         + ("_extended" if extra else ""))
...
for _f in (fig01, fig02, fig02ext, fig03, fig04, fig05, fig06, fig07, fig08,
           fig09, fig10, fig11, fig11ext, fig12, fig12ext):
    register(_f)
