...
The TESSERA embedding-generation pipeline produces embeddings tile-by-tile on
the standard global 0.1° grid. To extract an H×W patch centred at each weather
station (``scripts/data/extract_tessera_patches_local.py``), we need every tile
that any station's patch bounding box touches. This script reproduces that
extraction-time geometry and emits the deduplicated set of tiles to hand off to
the pipeline team (provisioning step; not needed once the mount is complete).

How it mirrors the extraction script
-------------------------------------
The extractor never names tiles itself — it builds a per-station WGS84 bounding
box and mosaics whichever 0.1° tiles overlap that box (geotessera's
``fetch_mosaic_for_region`` semantics). We replicate the two halves of that:

  1. **Patch bbox** — identical formula to ``compute_bbox`` in
     ``extract_tessera_patches_local.py``: a half-patch spans
     ``(patch_size / 2) * pixel_res_m`` metres, inflated by a 1.2× safety
     margin, converted to degrees with a latitude-aware longitude scaling
     (and the same ``max(..., 1000)`` clamp near the poles).
...
Usage (from the repo root):
    uv run python scripts/data/shortlist_tessera_tiles.py \
...
def compute_patch_bbox(
    ...
    """WGS84 bbox around a point — same formula as the extractor's ``compute_bbox``."""
