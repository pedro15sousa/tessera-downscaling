"""Extract TESSERA patch embeddings for every station from a local mount.

Produces the station patch tensor the VAE patch encoder is trained on and
encodes (``processed/tessera_station_patches/patch_embeddings_2017_p128.npy`` on
the data root: 38,870 stations x 128 x 128 x 128 channels, float32).

Context
-------
The TESSERA v2 embeddings are mounted read-only (``$TESSERA_V2_MOUNT``, default
``/tessera/v2/global_0.1_degree_representation``) as

    <mount>/<year>/grid_<lon>_<lat>/
...
mount does NOT ship landmasks, so they are looked up in a local cache
(``--landmask-cache``, default ``<data root>/_cache/geotessera/global_0.1_degree_tiff_all``)
and, for any gaps, downloaded (~13 KB each) from the version-independent v1
endpoint ``https://dl2.geotessera.org/v1/global_0.1_degree_tiff_all/<grid>.tiff``
(v1 landmask dims were verified to equal the v2 embedding dims).

The result is byte-for-byte identical to geotessera's
``fetch_mosaic_for_region(bbox, year, target_crs="EPSG:4326")`` followed by a
centre-crop (verified against geotessera for single-tile, 2-tile and 4-tile
corner patches), but reads embeddings straight from the mount, does no tile
downloads, and never deletes or copies mount data. ``compute_bbox`` /
``slice_patch`` below ARE that reference geometry.
...
Note: ``station_list_filtered.csv`` is just the (renamed) input station list; it
is NOT filtered by patch validity -- that check happens downstream in
``tessera_downscaling.data.helpers.filter_stations_by_tessera_patches``. A
``<npy>.progress.json`` sidecar records completed station indices so a run can
...
PIXEL_RES_M = 10.0
BBOX_MARGIN = 1.2  # safety margin on the patch bbox (geotessera reference geometry)
...
def compute_bbox(lon: float, lat: float, patch_size: int) -> tuple[float, float, float, float]:
    ...
    converted to degrees with a latitude-aware longitude scale (clamped near
    the poles). This is the bbox handed to geotessera in the reference
    extraction, so the set of tiles and the resulting mosaic match it.
    """
...
def provision_landmasks(tile_names: set[str], cache_dir: Path) -> dict[str, Path]:
    """Return {tile_name: landmask_path}, downloading gaps from the v1 endpoint.

    Landmasks already in cache_dir are used as-is; missing ones are downloaded
    into it. Tiles whose landmask cannot be found or fetched (e.g. ocean tiles
    with no TESSERA coverage) are simply omitted; build_mosaic skips them.
    """
    cache_dir.mkdir(parents=True, exist_ok=True)
    resolved: dict[str, Path] = {}
    missing: list[str] = []
    for name in sorted(tile_names):
        p = cache_dir / f"{name}.tiff"
        if p.exists():
            resolved[name] = p
        else:
            missing.append(name)
