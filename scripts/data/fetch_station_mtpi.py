...
Output:
    ...
    station. Feed it to ``scripts/preprocessing/preprocess_timestamp_global.py``
    via ``--mtpi-csv`` (which calls
...
def main() -> int:
    ...
    try:
        ...
        logger.error(
            "earthengine-api is not installed. Run `uv sync --extra ingest` first."
        )
        return 1
