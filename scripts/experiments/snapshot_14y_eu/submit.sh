...
# projects/tessera_downscaling/scripts/experiments/snapshot_14y_eu/experiments.yaml
...
#
# Output dir: .tmp_output/training_runs_snapshot_14y_eu/
#
...
#   bash projects/tessera_downscaling/scripts/experiments/snapshot_14y_eu/submit.sh
#
# Dry run (prints sbatch commands without submitting):
#   DRY_RUN=1 bash projects/tessera_downscaling/scripts/experiments/snapshot_14y_eu/submit.sh
set -euo pipefail
...
export VAE_LATENTS_PATH_LAT64_SHUFFLED="${BASE_DIR}/processed/station_latents_lat64_l1_shuffle_seed0.npy"

# Hand-crafted summary statistics over TESSERA patches — non-learned
# alternative to the VAE latents, built by build_summary_stats_latents.py.
# Row-aligned with VAE_LATENTS_CSV, loaded via the same --vae-latents-path
# code path (the trainer treats them identically).
export VAE_LATENTS_PATH_STATS_DIM16="${BASE_DIR}/processed/station_summary_stats_dim16.npy"
export VAE_LATENTS_PATH_STATS_DIM64="${BASE_DIR}/processed/station_summary_stats_dim64.npy"

# JEPA-encoder latents (pre-trained JEPA models, lambda=1.0 / mask=0.5, no
# auxiliary loss). Row-aligned with VAE_LATENTS_CSV (38,870 stations) and
# loaded through the same --vae-latents-path code path as the VAE latents.
# These files ship without a *_global_stats.npz sibling, so per-dim z-score
# stats are computed from all valid rows and cached on first use.
export VAE_LATENTS_PATH_JEPA128="${BASE_DIR}/processed/station_latents_jepa_lat128_lam1.0_mask0.5_noaux.npy"
export VAE_LATENTS_PATH_JEPA256="${BASE_DIR}/processed/station_latents_jepa_lat256_lam1.0_mask0.5_noaux.npy"

# Hand-crafted extra surface descriptors (Bakketun et al. 2026 style:
# land-cover fractions / canopy height / soil texture at the TESSERA patch
# footprint + topographic neighbourhood stats at the paper's 12.5 km kernel).
# Built by fetch_station_extra_descriptors.py + build_extra_descriptors.py.
# Passed via --extra-descriptors-path; when combined with --vae-latents-path
# in one run, train.py concatenates the two vectors internally (the derived
# _plus_ cache file + its stats are pre-built by concat_station_vectors.py
# so concurrent jobs only read).
export EXTRA_DESCRIPTORS_PATH="${BASE_DIR}/processed/extra_descriptors.npy"

OUTPUT_ROOT="${BASE_DIR}/training_runs_snapshot_14y_eu"
...
for experiment in "${EXPERIMENTS[@]}"; do
    ...
    for seed in "${SEEDS[@]}"; do
        ...
        if [ -n "${baseline_kind}" ]; then
            ...
            # extra_args is forwarded so baseline-specific flags can live in
            # experiments.yaml alongside everything else — e.g. the
            # era5_interp_lapse rows carry --lapse-rate-mode there. Trained
            # runs already consume extra_args via COMMON_ARGS; without this
            # the simple-baseline path silently dropped it.
