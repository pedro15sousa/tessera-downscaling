#!/usr/bin/env bash
# Submit the TESSERA 1B-M (2017 embeddings) arm for region "us": the
...
# the baseline folder snapshot_14y_us; runs land in
# <data root>/training_runs/snapshot_14y_us_tessera_1B-M_2017/<name>_seed<S>.
#
# Usage (from anywhere; data root from $TESSERA_DATA_ROOT):
#   bash scripts/experiments/snapshot_14y_us_tessera_1B-M_2017/submit.sh
#   DRY_RUN=1 bash scripts/experiments/snapshot_14y_us_tessera_1B-M_2017/submit.sh    # print the commands only
#   LOCAL=1   bash scripts/experiments/snapshot_14y_us_tessera_1B-M_2017/submit.sh    # run sequentially in this shell
