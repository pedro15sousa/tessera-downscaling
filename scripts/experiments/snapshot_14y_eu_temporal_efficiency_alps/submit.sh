#!/bin/bash
# Submit the Alps-clustered-probe variant of the temporal-axis data-
# efficiency experiment for EU 14y.
#
# This is identical in structure to ../snapshot_14y_eu_temporal_efficiency/
# (same sweep_points, same architectures, same seeds), but with a probe
# set that is geographically clustered in the high Alps (lat 45-48, lon
# 6-14, elev > 800m) rather than randomly scattered across all of EU.
#
# The probe_station_ids.json file in this folder is HAND-PICKED — pre-
# generated from a notebook by intersecting the bbox+elev filter with
# the TESSERA+VAE filter, then dumping the surviving station_ids. This
# submit script EXPECTS that file to exist; it does not generate one
# itself (unlike its v1 sibling which deterministically picks a random
# 5% set on first run).
#
# Pre-flight steps performed inline by this script:
#   1. Validates probe_station_ids.json exists in this folder, with the
#      expected JSON shape. Fails loudly if missing — re-generate it
#      from the notebook cell rather than re-running the v1 random-pick
#      logic.
#   2. For each sweep_point, writes a ``probe_active_from_{label}.json``
#      mapping every probe station_id to that point's active_from
#      timestamp. Cached: existing files left in place.
#   3. Issues one sbatch per (architecture, sweep_point, seed) plus one
#      sbatch per simple_baseline (no x-sweep, no model seed).
#
# Usage (from repo root):
#   bash projects/tessera_downscaling/scripts/experiments/snapshot_14y_eu_temporal_efficiency_alps/submit.sh
#
# Dry run:
#   DRY_RUN=1 bash projects/tessera_downscaling/scripts/experiments/snapshot_14y_eu_temporal_efficiency_alps/submit.sh
set -euo pipefail
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${BASE_DIR}/training_runs_snapshot_14y_eu_temporal_efficiency_alps}"
...
# ---- Step 1: Validate the hand-picked probe set ----
# Unlike the v1 sibling experiment (which deterministically picks a
# random 5% probe set), this Alps variant uses a probe set hand-picked
# in a notebook (intersect Alps bbox + elev floor with TESSERA+VAE
# filter, dump station_ids). The submit script just sanity-checks the
...
if [ ! -f "${PROBE_IDS_JSON}" ]; then
    ...
    echo "       Generate it from the Alps-probe selection notebook cell"     >&2
