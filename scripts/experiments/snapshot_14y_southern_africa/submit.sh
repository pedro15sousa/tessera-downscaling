#!/usr/bin/env bash
# Submit the baseline arms for region "southern_africa" of the paper: no-model references
...
# --train-regions southern_africa (per-region normalisation stats); runs land in
# <data root>/training_runs/snapshot_14y_southern_africa/<name>_seed<S>.
#
# Usage (from anywhere; data root from $TESSERA_DATA_ROOT):
#   bash scripts/experiments/snapshot_14y_southern_africa/submit.sh              # sbatch one job per (entry, seed)
#   DRY_RUN=1 bash scripts/experiments/snapshot_14y_southern_africa/submit.sh    # print the commands only
#   LOCAL=1   bash scripts/experiments/snapshot_14y_southern_africa/submit.sh    # run sequentially in this shell
