#!/bin/bash
# Submit the southern_africa TESSERA-1B-M VAE-latent sweep (embedding year
...
#
# Output dir: .tmp_output/training_runs_snapshot_14y_southern_africa_tessera_1B-M_2017/
#
# Usage (from repo root) — DRY RUN IS THE DEFAULT for this folder:
#   bash projects/tessera_downscaling/scripts/experiments/snapshot_14y_southern_africa_tessera_1B-M_2017/submit.sh
# Real submission:
#   DRY_RUN=0 bash projects/tessera_downscaling/scripts/experiments/snapshot_14y_southern_africa_tessera_1B-M_2017/submit.sh
set -euo pipefail
...
# Hand-crafted extra surface descriptors (17 dims; Bakketun et al. 2026
# style). Combined with --vae-latents-path in one run, train.py hstacks the
# two vectors into a derived cache next to the latents file.
export EXTRA_DESCRIPTORS_PATH="${BASE_DIR}/processed/extra_descriptors.npy"
# Patch summary-statistics control (Appendix A): 16-d hand-crafted stats over
# the same crop64 window of the same patches, built by
# build_summary_stats_latents.py --center-crop 64 (NaN-mask aligned to the
# crop64_lat16_auxon latents).
export SUMMARY_STATS_PATH="${BASE_DIR}/processed/station_summary_stats_${TESSERA_VARIANT}_p128_${EMBED_YEAR}_crop64_dim16.npy"

OUTPUT_ROOT="${BASE_DIR}/training_runs_snapshot_14y_southern_africa_tessera_1B-M_2017"
