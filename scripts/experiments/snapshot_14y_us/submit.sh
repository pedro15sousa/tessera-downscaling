#!/usr/bin/env bash
# Submit the baseline arms for region "us" of the paper: no-model references
...
# --train-regions us (per-region normalisation stats); runs land in
# <data root>/training_runs/snapshot_14y_us/<name>_seed<S>.
#
# Usage (from anywhere; data root from $TESSERA_DATA_ROOT):
#   bash scripts/experiments/snapshot_14y_us/submit.sh              # sbatch one job per (entry, seed)
#   DRY_RUN=1 bash scripts/experiments/snapshot_14y_us/submit.sh    # print the commands only
#   LOCAL=1   bash scripts/experiments/snapshot_14y_us/submit.sh    # run sequentially in this shell
