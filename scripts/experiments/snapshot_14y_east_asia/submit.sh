#!/usr/bin/env bash
# Submit the baseline arms for region "east_asia" of the paper: no-model references
...
# --train-regions east_asia (per-region normalisation stats); runs land in
# <data root>/training_runs_snapshot_14y_east_asia/<name>_seed<S>.
#
# Usage (from anywhere; data root from $TESSERA_DATA_ROOT):
#   bash scripts/experiments/snapshot_14y_east_asia/submit.sh              # sbatch one job per (entry, seed)
#   DRY_RUN=1 bash scripts/experiments/snapshot_14y_east_asia/submit.sh    # print the commands only
#   LOCAL=1   bash scripts/experiments/snapshot_14y_east_asia/submit.sh    # run sequentially in this shell
