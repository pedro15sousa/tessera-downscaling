...
3. **The canonical VAE latent is `crop64_lat16_grad0.5_auxon`** — the patch-encoder repo's `aux` flag turns on auxiliary elevation / lat / lon prediction heads during VAE training. App. A.3 / Table A5 describe only recon + gradient + KL losses. Verify what `aux` does in `tessera-patch-encoder/scripts/vae/train.py` and document it (a lat/lon auxiliary head is worth an explicit sentence).
