...
| `cf6f17d` | TESSERA patch-encoder VAE integrated (`patch_encoder/`), paper checkpoint strict-loads, latents reproduce to 1.8e-6 |
| `6ad2bf1` | AlphaEarth / OlmoEarth benchmark arms restored (`scripts/patch_encoder/extract/`, FM configs, sweep), `fm` extra |
| `aba7555` | LICENSE (MIT, Pedro Sousa + imported-portions notice) — pushed to GitHub |
...
3. ~~Push to a new remote~~ — **done 2026-08-20**: `git@github.com:pedro15sousa/tessera-downscaling.git`, branch `main` (23 commits), MIT licence, commit author kept as the personal address. The two GitHub workflows (CI: `uv sync --group dev` + pytest — first run downloads the cu126 torch wheel; pre-commit) trigger on push. **The monorepo copy is kept** (author's decision) — Phase 6 is dropped; `projects/tessera_downscaling` in `end-to-end-forecasting` remains as the historical tree (and still hosts the untracked `wind_energy/` code until its own migration). Note for this machine: `~/.ssh/config` routes GitHub SSH to `ssh.github.com:443`; when that path times out, push with `GIT_SSH_COMMAND="ssh -F /dev/null -o Port=22 -o UserKnownHostsFile=<file with github.com ed25519 key, fingerprint SHA256:+DiY3wvvV6TuJJhbpZisF/zLDA0zPMSvHdkr4UvCOqU>"`.
