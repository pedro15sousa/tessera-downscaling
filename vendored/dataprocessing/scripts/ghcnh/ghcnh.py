...
# ROOT = Path(".tmp_output")
ROOT = Path("projects/tessera_downscaling/.tmp_output/_staging")
RAW_ROOT = ROOT / "raw" / "ghcnh"
PROCESSED_ROOT = ROOT / "processed" / "ghcnh"
...
def main() -> None:
    ...
    with station_path.open("rb") as f:
        ...
        # # Filter to European stations matching the bounding box from Vaughan et al. (2022).
        # eu_mask = (
        #     (station_df["LATITUDE"] >= 35) & (station_df["LATITUDE"] <= 75)
        #     & (station_df["LONGITUDE"] >= -24) & (station_df["LONGITUDE"] <= 40)
        # )
        # station_ids = station_df[eu_mask]["GHCN_ID"].to_numpy()
        station_ids = station_df["GHCN_ID"].to_numpy()
    ...
    for year in range(2010, 2024):
        ...
        # and writing it to blob storage. Comment this out if you already have the data.
