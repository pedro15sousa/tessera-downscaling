...
Output:
    ...
    ``projects/tessera_downscaling/scripts/preprocessing/build_extra_descriptors.py``
    to produce the row-aligned ``extra_descriptors.npy`` for training.
...
Usage (from repo root, using the shared workspace venv):
    .venv/bin/python projects/dataprocessing/scripts/gee/fetch_station_extra_descriptors.py \
        --stations-csv projects/tessera_downscaling/.tmp_output/processed/tessera_global/station_list_filtered.csv \
        --output-csv   projects/tessera_downscaling/.tmp_output/processed/station_extra_descriptors.csv
