...
Used by both `cross_folder_analysis.ipynb` (loads all folders, compares
across them) and `single_folder_analysis.ipynb` (loads one folder, does
detailed analysis on it).

Source of truth for "what experiments belong to a folder" is each
folder's `experiments.yaml`. Source of truth for "what runs exist on
disk" is each folder's training_runs_<folder_name>/ directory.
"""
...
def find_repo_root(start: Path | None = None) -> Path:
    """Walk up from `start` (or cwd) until we find projects/tessera_downscaling.

    Mirrors the path-discovery cell at the top of the original notebook.
    """
    p = (start or Path.cwd()).resolve()
    while not (p / "projects" / "tessera_downscaling").exists() and p != p.parent:
        p = p.parent
    if not (p / "projects" / "tessera_downscaling").exists():
        raise RuntimeError(
            f"Could not locate projects/tessera_downscaling above {start or Path.cwd()}"
        )
    return p


def experiments_dir(repo_root: Path | None = None) -> Path:
    """Path to projects/tessera_downscaling/scripts/experiments/."""
    repo_root = repo_root or find_repo_root()
    return repo_root / "projects" / "tessera_downscaling" / "scripts" / "experiments"
...
def build_experiment_defs(folders: Iterable[str], repo_root: Path | None = None) -> dict:
    """Replacement for the old hardcoded EXPERIMENT_DEFS dict.

    Keyed by experiment name; value is a dict with label, colour, and
    variables (list of target-variable names — the field is named
    `variables` for compatibility with the rest of the notebook code,
    even though the YAML calls it `target_variables`).

    Note: experiment names are intentionally identical across the 6
    sibling YAMLs (the 28-config matrix is shared). When that happens,
    the LATER folder's metadata wins — but since labels and colours
    are identical too, this is a no-op in practice. The drift-check
    in the README catches any divergence.

    For folder-specific lookup of run dirs (i.e. which folders ran
    this experiment), use `build_run_index` instead.
    """
    defs = {}
...
def build_run_index(folders: Iterable[str], repo_root: Path | None = None) -> dict:
    """Map experiment-name -> set of folders that include it.

    Useful for cross-folder analysis: tells you which folders to look
    in when fetching results for a given experiment name.
    """
    idx: dict[str, set[str]] = defaultdict(set)
    for folder in folders:
        for entry in load_experiments_yaml(folder, repo_root):
            idx[entry["name"]].add(folder)
    return dict(idx)
...
# Backwards-compatible flat list, kept so any external caller that
# imported _PER_VARIABLE_METRICS doesn't break. Returns the *Gaussian*
# metric set since that's the legacy assumption.
_PER_VARIABLE_METRICS = _COMMON_METRICS + _DISTRIBUTION_METRICS["gaussian"]
...
# preprocessing/timestamp/preprocess_timestamp_global.py. Areas are
...
# =========================================================================
# Globally-trained model (scoping doc §3.2.2)
# =========================================================================
# One checkpoint per config is trained on all five regions, then evaluated
# PER REGION into {run_dir}/eval_{region}/test_summary.json. The standard
# loaders assume one summary per run dir and infer region from the folder,
# so global runs need a dedicated loader that (a) descends into each
# eval_{region}/ subdir and (b) tags every row with an explicit `region` and
# `training_scope="global"` — making global rows unambiguously distinct from
# the per-region-trained models without relying on the folder→region map.

GLOBAL_EVAL_REGIONS = ("europe", "us", "east_asia", "australia", "southern_africa")

# Maps an eval region to the folder holding its per-region-trained comparands.
_PER_REGION_FOLDER = {
...
def load_global_results(
    folder: str = "snapshot_global_14y_all",
    seeds: Iterable[int] = (42, 123, 456),
    regions: Iterable[str] = GLOBAL_EVAL_REGIONS,
    repo_root: Path | None = None,
) -> pd.DataFrame:
    """Load a globally-trained model's per-region evaluations.

    Returns one row per (config, seed, eval-region). Each row carries an
    explicit ``region`` column (the region it was *evaluated* on) and
    ``training_scope="global"``. Shares the column schema of
    ``load_folder_results`` so global rows can be concatenated with the
    per-region dataframe and compared directly.
    """
    repo_root = repo_root or find_repo_root()
    output_dir = output_dir_for_folder(folder, repo_root)
    if not output_dir.exists():
    ...
    for entry in load_experiments_yaml(folder, repo_root):
        ...
        for seed in seeds:
            ...
            for region in regions:
                eval_dir = run_dir / f"eval_{region}"
                ...
                if rec is not None:
                    rec["region"] = region
                    rec["training_scope"] = "global"
                    records.append(rec)
    return pd.DataFrame(records)


# Per-variable comparand selection: the exact per-region experiment names the
# global model is compared against (the non-mTPI headline TESSERA config the
# global model mirrors, its trained no-TESSERA twin, and the two non-trained
# baselines). Exact names — not substrings — so the generative / CRPS / mTPI
# siblings present in the per-region folders are not silently averaged in. A
# name absent from a region's folder simply prints "—".
_COMPARAND_RULES = {
    "t2m": [
        ("per-region TESSERA",  "t2m_snap_vae_lat16_concat_with_elev_no_static_wd"),
        ("per-region baseline", "t2m_snap_bilinear_baseline_wd"),
        ("ERA5 interp",         "t2m_snap_era5_interp_baseline"),
        ("persistence",         "t2m_snap_persistence_baseline"),
    ],
    "wind": [
        ("per-region TESSERA",  "wind_truncnormal_snap_vae_lat16_concat_with_elev_no_static_wd"),
        ("per-region baseline", "wind_truncnormal_snap_bilinear_baseline_wd"),
...
def _filter_by_name(df: pd.DataFrame, *, contains: str | None = None,
                    equals: str | None = None, pred=None) -> pd.DataFrame:
    """Filter rows by experiment name, safe on empty / columnless frames."""
    if df is None or len(df) == 0 or "experiment" not in getattr(df, "columns", []):
        return pd.DataFrame()
    names = df["experiment"].astype(str)
    if equals is not None:
        return df[names == equals]
    if contains is not None:
        return df[names.str.contains(contains)]
    if pred is not None:
        return df[names.apply(pred)]
    return df
...
def global_vs_per_region(
    variable: str,
    *,
    mtpi: str = "off",
    global_folder: str = "snapshot_global_14y_all",
    seeds: Iterable[int] = (42, 123, 456),
    regions: Iterable[str] = GLOBAL_EVAL_REGIONS,
    metrics: Iterable[str] = ("point_mae", "point_rmse", "nll", "crps"),
    repo_root: Path | None = None,
    do_print: bool = True,
) -> pd.DataFrame:
    """Per-region comparison of the globally-trained model vs every counterpart.

    For each region, lays out (mean ± std over seeds) for:
      * Global TESSERA            — the §3.2.2 model, this region's eval
      * Global baseline           — same, trained no-TESSERA twin
      * per-region TESSERA         — the matching per-region-trained headline
      * per-region baseline        — per-region-trained no-TESSERA twin
      * ERA5 interp / persistence  — the non-trained references
    plus ``d_point_mae_vs_region`` = global − per-region TESSERA (the
    "one model, anywhere" gap; ≤ 0 means global matches or beats per-region).

    ``mtpi`` selects which global variant to show ("off" or "on"). Returns a
    tidy dataframe (one row per region × method) and optionally prints it.
    """
    repo_root = repo_root or find_repo_root()
    gdf = load_global_results(global_folder, seeds=seeds, regions=regions, repo_root=repo_root)
    if not gdf.empty:
        is_mtpi = gdf["experiment"].str.contains("mtpi")
        gdf = gdf[is_mtpi if mtpi == "on" else ~is_mtpi]
        gdf = gdf[gdf["variables"].apply(lambda vs: vs == [variable])]

    rules = _COMPARAND_RULES.get(variable, [])
    metric_cols = [f"{variable}_{m}" for m in metrics]
    out_rows = []

    for region in regions:
        g_region = gdf[gdf["region"] == region] if not gdf.empty else pd.DataFrame()
        g_tessera = _filter_by_name(g_region, contains="vae")
        g_baseline = _filter_by_name(g_region, contains="baseline")

        pr = load_folder_results(_PER_REGION_FOLDER.get(region, ""), seeds=seeds, repo_root=repo_root)
        if not pr.empty:
            pr = pr[pr["variables"].apply(lambda vs: vs == [variable])]

        per_region_tessera_mae = None
        bundle = [
            ("global TESSERA", g_tessera),
            ("global baseline", g_baseline),
        ]
        for label, name in rules:
            bundle.append((label, _filter_by_name(pr, equals=name)))

        for label, rows in bundle:
            row = {"region": region, "method": label}
            for m, col in zip(metrics, metric_cols):
                mean, std, n = _agg_metric(rows, col)
                row[m] = mean
                row[f"{m}_std"] = std
                if m == "point_mae":
                    row["n_seeds"] = n
            if label == "per-region TESSERA":
                per_region_tessera_mae = row["point_mae"]
            out_rows.append(row)

        # Fill the "one model, anywhere" gap once the per-region anchor is known.
        for row in out_rows:
            if row["region"] == region and row["method"] == "global TESSERA":
                row["d_point_mae_vs_region"] = (
                    row["point_mae"] - per_region_tessera_mae
                    if per_region_tessera_mae is not None else float("nan")
                )

    df = pd.DataFrame(out_rows)

    if do_print and not df.empty:
        print(f"\n=== Global vs per-region — {variable} (global mTPI={mtpi}) ===")
        print("    point_mae / point_rmse / nll / crps : mean (±std over seeds)\n")
        for region in regions:
            ...
            for _, r in sub.iterrows():
                cells = []
                for m in metrics:
                    mv, sv = r.get(m, float("nan")), r.get(f"{m}_std", float("nan"))
                    cells.append(f"{m}={mv:6.3f}±{sv:4.2f}" if mv == mv else f"{m}=  —  ")
                tag = ""
                if r["method"] == "global TESSERA" and r.get("d_point_mae_vs_region") == r.get("d_point_mae_vs_region"):
                    tag = f"   Δmae_vs_region={r['d_point_mae_vs_region']:+.3f}"
                print(f"    {r['method']:<20} n={int(r.get('n_seeds', 0) or 0):<2} " + "  ".join(cells) + tag)
            print()
    return df


# =========================================================================
# Capacity sweep (bigger CNN)
# =========================================================================
# Compares the base CNN (128×7 k3 = 3 residual blocks) against the two larger
# variants — cnn128x13k5 (Vaughan-depth: 6 blocks, kernel 5) and cnn256x13k5
# (deep + wide) — for the lat16-direct-concat TESSERA model and its no-TESSERA
# baseline, per region. Runs are identified purely by the cnn{H}x{L}k{K} tag in
# the experiment name, so this works the moment the capacity runs land and
# shows "—" for those not yet finished.

# Ordered (display label, name-suffix) for the three capacity points.
CAPACITY_SIZES = (
    ("cnn128x7k3 (base)", ""),
    ("cnn128x13k5",       "_cnn128x13k5"),
    ("cnn256x13k5",       "_cnn256x13k5"),
)


def _capacity_exp_name(scheme: str, variable: str, model_type: str,
                       mtpi: bool, size_suffix: str) -> str:
    """Reconstruct an experiment name across the {scheme, model, mTPI, size} axes.

    Mirrors the naming used in the experiment YAMLs exactly: the size tag sits
    immediately before ``_wd``; mTPI is encoded per-scheme (per-region tessera
    embeds it mid-name as ``with_elev_mtpi_no_static``; per-region baseline and
    the global scheme append ``_mtpi``).
    """
    m = "_mtpi" if mtpi else ""
    if scheme == "global":
        core = "t2m_global" if variable == "t2m" else "wind_truncnormal_global"
        if model_type == "tessera":
            return f"{core}_vae_lat16_concat{m}{size_suffix}_wd"
        return f"{core}_bilinear_baseline{m}{size_suffix}_wd"
    # per-region "snap" scheme
    pre = "t2m_snap" if variable == "t2m" else "wind_truncnormal_snap"
    if model_type == "tessera":
        return f"{pre}_vae_lat16_concat_with_elev{m}_no_static{size_suffix}_wd"
    return f"{pre}_bilinear_baseline{m}{size_suffix}_wd"
...
def capacity_view(
    ...
    *,
    ...
    model_types: Iterable[str] = ("tessera", "baseline"),
    mtpi_values: Iterable[bool] = (False, True),
    metric: str = "point_mae",
    metrics: Iterable[str] = ("point_mae", "point_rmse", "nll", "crps"),
    seeds: Iterable[int] = (42, 123, 456),
    repo_root: Path | None = None,
    do_print: bool = True,
) -> pd.DataFrame:
    """Per-region capacity ladder: base vs cnn128x13k5 vs cnn256x13k5.

    For each region (and, if ``include_global``, the global model's eval on that
    region) × model_type × mTPI, aggregates each size's metrics over seeds and
    reports ``d_point_mae_vs_base`` = size − base MAE (≤ 0 ⇒ the bigger model
    helps). Returns a tidy dataframe; ``do_print`` adds a compact per-region
    MAE-ladder summary. Sizes with no finished runs read "—".
    """
    repo_root = repo_root or find_repo_root()
    metrics = tuple(dict.fromkeys((metric, *metrics)))  # ensure the printed metric is aggregated
    metric_cols = [f"{variable}_{m}" for m in metrics]

    folder_cache: dict[str, pd.DataFrame] = {}

    def region_df(region):
        if region not in folder_cache:
            folder_cache[region] = load_folder_results(
                _PER_REGION_FOLDER.get(region, ""), seeds=seeds, repo_root=repo_root)
        return folder_cache[region]

    gdf = (load_global_results(seeds=seeds, regions=regions, repo_root=repo_root)
           if include_global else pd.DataFrame())

    scopes = [("per_region", r) for r in regions]
    if include_global:
        scopes += [("global", r) for r in regions]

    rows = []
    for scope, region in scopes:
        for model_type in model_types:
            for mtpi in mtpi_values:
                base_mae = None
                for size_label, suffix in CAPACITY_SIZES:
                    name = _capacity_exp_name(scope, variable, model_type, mtpi, suffix)
                    if scope == "global":
                        sel = (gdf[(gdf["region"] == region) & (gdf["experiment"] == name)]
                               if not gdf.empty and "experiment" in gdf.columns else pd.DataFrame())
                    else:
                        sel = _filter_by_name(region_df(region), equals=name)
                    rec = {"scope": scope, "region": region, "variable": variable,
                           "model": model_type, "mtpi": "on" if mtpi else "off",
                           "size": size_label, "experiment": name}
                    for m, col in zip(metrics, metric_cols):
                        mean, std, n = _agg_metric(sel, col)
                        rec[m], rec[f"{m}_std"] = mean, std
                        if m == "point_mae":
                            rec["n_seeds"] = n
                    if suffix == "":
                        base_mae = rec["point_mae"]
                    rec["d_point_mae_vs_base"] = (
                        rec["point_mae"] - base_mae
                        if base_mae is not None and base_mae == base_mae else float("nan"))
                    rows.append(rec)

    df = pd.DataFrame(rows)

    if do_print and not df.empty:
        print(f"\n=== Capacity sweep — {variable} : {metric} mean over seeds, Δ vs base "
              f"({'lower better' if metric != 'correlation' else 'higher better'}) ===")
        for scope, region in scopes:
            sub = df[(df["scope"] == scope) & (df["region"] == region)]
            if sub.empty or sub[metric].notna().sum() == 0:
                continue
            label = region if scope == "per_region" else f"{region} (GLOBAL model)"
            print(f"  [{label}]")
            for model_type in model_types:
                for mtpi in mtpi_values:
                    line = sub[(sub["model"] == model_type) & (sub["mtpi"] == ("on" if mtpi else "off"))]
                    base_val = next((r.get(metric) for _, r in line.iterrows()
                                     if r["size"].startswith("cnn128x7k3")), None)
                    cells = []
                    for _, r in line.iterrows():
                        mv = r.get(metric, float("nan"))
                        if mv != mv:
                            cells.append(f"{r['size']}=  —  ")
                        elif r["size"].startswith("cnn128x7k3"):
                            cells.append(f"{r['size']}={mv:.3f}")
                        else:
                            d = (mv - base_val) if (base_val is not None and base_val == base_val) else float("nan")
                            cells.append(f"{r['size']}={mv:.3f}" + (f"(Δ{d:+.3f})" if d == d else ""))
                    print(f"    {model_type:<8} mtpi={'on ' if mtpi else 'off'} | " + "  ".join(cells))
            print()
    return df
...
def capacity_convergence(
    ...
    *,
    ...
    """Convergence / early-stop triage for the per-region capacity runs.

    Reads each run's ``training_summary.json`` (``n_epochs_run``, best/final
    val loss) and ``test_summary.json`` (``checkpoint_epoch`` = best epoch),
    averaged over seeds, to tell apart two reasons a larger model's test MAE
    could be worse:
      * OVERFITTING   — early-stopped before the cap, ``final_val`` rose above
        ``best_val`` (positive ``overfit_gap``).
      * UNDER-TRAINING — ran to the epoch cap (flagged ``[CAP]``): the LR/
        schedule may just be too slow for the bigger model.
    A lower ``best_val_loss`` at a larger size corroborates a genuine gain; a
    higher one corroborates real degradation.
    """
    import numpy as _np
    repo_root = repo_root or find_repo_root()
    ...
    def _mean(xs):
        ...
        return float(_np.mean(xs)) if xs else float("nan")

    rows = []
    for region in regions:
        out_dir = output_dir_for_folder(_PER_REGION_FOLDER.get(region, ""), repo_root)
        for model_type in model_types:
            for mtpi in mtpi_values:
                for size_label, suffix in CAPACITY_SIZES:
                    name = _capacity_exp_name("per_region", variable, model_type, mtpi, suffix)
                    epochs, best_vl, final_vl, best_ep, gaps, n = [], [], [], [], [], 0
                    for seed in seeds:
                        rd = out_dir / f"{name}_seed{seed}"
                        try:
                            t = json.load(open(rd / "training_summary.json"))
                        except (FileNotFoundError, json.JSONDecodeError, ValueError):
                            continue
                        n += 1
                        epochs.append(t.get("n_epochs_run"))
                        bv, fv = t.get("best_val_loss"), t.get("final_val_loss")
                        best_vl.append(bv); final_vl.append(fv)
                        if isinstance(bv, (int, float)) and isinstance(fv, (int, float)):
                            gaps.append(fv - bv)
                        try:
                            best_ep.append(json.load(open(rd / "test_summary.json")).get("checkpoint_epoch"))
                        except (FileNotFoundError, json.JSONDecodeError, ValueError):
                            pass
                    rows.append({
                        "region": region, "variable": variable, "model": model_type,
                        "mtpi": "on" if mtpi else "off", "size": size_label, "n_seeds": n,
                        "epochs_run": _mean(epochs), "best_epoch": _mean(best_ep),
                        "best_val_loss": _mean(best_vl), "final_val_loss": _mean(final_vl),
                        "overfit_gap": _mean(gaps),
                    })
    df = pd.DataFrame(rows)

    if do_print and not df.empty:
        print(f"\n=== Capacity convergence — {variable} (epoch cap {epoch_cap}; "
              f"[CAP]=ran to cap → maybe under-trained; gap=final−best val) ===")
        for region in regions:
            sub = df[df["region"] == region]
            if sub.empty or sub["epochs_run"].notna().sum() == 0:
                continue
            print(f"  [{region}]")
            for model_type in model_types:
                for mtpi in mtpi_values:
                    for _, r in sub[(sub["model"] == model_type)
                                    & (sub["mtpi"] == ("on" if mtpi else "off"))].iterrows():
                        er = r["epochs_run"]
                        if er != er:
                            print(f"    {model_type:<8} mtpi={'on ' if mtpi else 'off'} {r['size']:<18}  —")
                            continue
                        cap = " [CAP]" if er >= epoch_cap - 0.5 else "      "
                        print(f"    {model_type:<8} mtpi={'on ' if mtpi else 'off'} {r['size']:<18} "
                              f"epochs={er:5.1f}{cap}  best_ep={r['best_epoch']:5.1f}  "
                              f"best_val={r['best_val_loss']:7.4f}  gap={r['overfit_gap']:+.4f}")
            print()
    return df
...
def get_baseline_and_shortlist(
    shortlist: list[str],
    variable: str,
    distribution: str = "gaussian",
) -> tuple[list[str], list[str]]:
    """Split shortlist into (baseline_names, tessera_only_shortlist).

    Resolves baselines for the (variable, distribution) pair via
    :func:`baselines_for`. For legacy callers that didn't pass a
    distribution, the default ``"gaussian"`` reproduces the original
    behaviour exactly.

    Raises ``ValueError`` if no baselines are registered for the pair —
    callers should pre-check ``baselines_for(variable, distribution)``
    if a missing-baseline case is expected (e.g. partial v4 sweeps).
    """
    baselines = baselines_for(variable, distribution)
    if not baselines:
        raise ValueError(
            f"No baselines registered for variable={variable!r}, "
            f"distribution={distribution!r}. Known: "
            f"{sorted(BASELINE_NAMES.keys())}."
        )
    tessera_only = [e for e in shortlist if e not in baselines]
    return baselines, tessera_only


# -----------------------------------------------------------------------------
# Cross-folder helpers
# -----------------------------------------------------------------------------

def cross_folder_table(
    df: pd.DataFrame,
    variable: str,
    experiments: Iterable[str] | None = None,
    distribution: str | None = None,
) -> pd.DataFrame:
    """Pivot results into a (experiment × source_folder) point-estimate table.

    The metric pivoted is the distribution-appropriate "MAE-equivalent" —
    Gaussian: ``mae``, Weibull: ``mae_at_median``, B-G: ``wet_mae_at_median``.
    Cells are formatted as ``"mean±std (n=k)"`` strings.

    If ``distribution`` is None, picks per-row using ``<variable>_distribution``
    in ``df`` (resolves correctly even when ``df`` mixes Gaussian and v4
    rows for the same variable, e.g. wind under both legacy and Weibull
    families). Pre-v4 rows default to "gaussian".

    If ``experiments`` is None, includes every experiment that has at
    least one non-NaN value in its row's MAE-equivalent column.
    """
    # Resolve the per-row MAE column. Three cases:
    #   (a) distribution explicitly given      → one column, simple filter.
    #   (b) distribution column on df          → row-wise dispatch.
    #   (c) neither                            → fall back to legacy "mae".
    dist_col = f"{variable}_distribution"

    if distribution is not None:
        mae_col_name = _point_estimate_mae_column(distribution)
        mae_col = f"{variable}_{mae_col_name}"
        if mae_col not in df.columns:
            return pd.DataFrame()
        sub = df[df[mae_col].notna()].copy()
        sub["_pe_mae"] = sub[mae_col]
    elif dist_col in df.columns:
        # Row-wise dispatch — gather the right column per row.
        def _pick(row):
            d = row[dist_col] if pd.notna(row[dist_col]) else "gaussian"
            col = f"{variable}_{_point_estimate_mae_column(d)}"
            return row.get(col)
        sub = df.copy()
        sub["_pe_mae"] = sub.apply(_pick, axis=1)
        sub = sub[sub["_pe_mae"].notna()]
    else:
        mae_col = f"{variable}_mae"
        if mae_col not in df.columns:
            return pd.DataFrame()
    ...
    grouped = sub.groupby(["experiment", "source_folder"])["_pe_mae"].agg(
        ["mean", "std", "count"]
    )
    ...
    table = grouped["formatted"].unstack(level="source_folder")

    # Add the readable label as the leftmost column.
    label_map = sub.drop_duplicates("experiment").set_index("experiment")["label"]
    table.insert(0, "label", table.index.map(label_map))
    return table


def baselines_across_folders(
    df: pd.DataFrame,
    variable: str,
    distribution: str = "gaussian",
) -> pd.DataFrame:
    """Subset of cross_folder_table restricted to the baseline experiments.

    Uses :func:`baselines_for` to resolve the right family-prefixed
    baseline names for the (variable, distribution) pair. For Gaussian
    wind that's ``wind_snap_*`` baselines; for Weibull wind that's
    ``wind_weibull_snap_*``; for B-G precip ``precip_snap_*``.

    Returns an empty DataFrame if no baselines are registered for this
    pair (e.g. legacy ``precip`` runs that don't exist yet).
    """
    baselines = baselines_for(variable, distribution)
    if not baselines:
        return pd.DataFrame()
    return cross_folder_table(
        df, variable, experiments=baselines, distribution=distribution,
    )


# -----------------------------------------------------------------------------
# Architecture meta-analysis (Item 5, Angle B)
# -----------------------------------------------------------------------------
...
def parse_experiment_name(name: str) -> dict:
    """Parse an experiment name into its architectural knobs.

    Returns a dict with these keys (None when not applicable):
      - target         ∈ {'t2m', 'wind', 'multitask'} or None
      - kind           ∈ {'tessera_vae', 'bilinear_baseline',
                          'era5_interp_baseline', 'persistence_baseline'} or None
      - latent_dim     ∈ {16, 64} or None
      - proj_dim       ∈ {0, 8, 16, 32} or None  (0 = direct, no projection)
      - proj_style     ∈ {'direct', 'linear', 'mlp'} or None
      - injection      ∈ {'concat', 'film'} or None
      - elev           ∈ {'with', 'no'} or None
      - static         ∈ {'with', 'no'} or None

    Names not following the conventions return all-None except 'target'.
    """
    out = {
        "target": None, "kind": None,
        "latent_dim": None, "proj_dim": None, "proj_style": None,
        "injection": None, "elev": None, "static": None,
    }

    # Target prefix
    if name.startswith("multitask_"):
        out["target"] = "multitask"
        rest = name[len("multitask_"):]
    elif name.startswith("t2m_"):
        out["target"] = "t2m"
        rest = name[len("t2m_"):]
    elif name.startswith("wind_"):
        out["target"] = "wind"
        rest = name[len("wind_"):]
    else:
        return out

    if rest.startswith("snap_"):
        rest = rest[len("snap_"):]

    # Baseline kinds
    if "bilinear_baseline" in rest:
        out["kind"] = "bilinear_baseline"
        out["elev"] = "with"
        out["static"] = "with"
        return out
    if "era5_interp_baseline" in rest:
        out["kind"] = "era5_interp_baseline"
        return out
    if "persistence_baseline" in rest:
        out["kind"] = "persistence_baseline"
        return out

    # TESSERA VAE configs
    if not rest.startswith("vae_"):
        return out
    out["kind"] = "tessera_vae"
    rest = rest[len("vae_"):]

    m = _re.match(
        r"lat(\d+)(?:_proj(\d+)(?:_(mlp))?)?_(concat|film)_(with|no)_elev_(with|no)_static_wd",
        rest,
    )
    if m is None:
        return out

    out["latent_dim"] = int(m.group(1))
    proj = m.group(2)
    out["proj_dim"] = int(proj) if proj else 0
    if proj is None:
        out["proj_style"] = "direct"
    elif m.group(3) == "mlp":
        out["proj_style"] = "mlp"
    else:
        out["proj_style"] = "linear"
    out["injection"] = m.group(4)
    out["elev"] = m.group(5)
    out["static"] = m.group(6)
    return out


def enrich_with_knobs(df: pd.DataFrame) -> pd.DataFrame:
    """Add architectural-knob columns (parsed from `experiment`) to `df`.

    Adds columns: target, kind, latent_dim, proj_dim, proj_style,
    injection, elev, static. Returns a new DataFrame; original is not
    modified. Idempotent — re-running on an already-enriched df just
    overwrites the columns.
    """
    out = df.copy()
    parsed = out["experiment"].apply(parse_experiment_name)
    parsed_df = pd.DataFrame(list(parsed))
    for col in parsed_df.columns:
        out[col] = parsed_df[col].values
    return out


# Knobs and their value sets used in marginal-effect analysis. Order
# matters for plotting (left-to-right axis ordering).
KNOB_VALUES = {
    "latent_dim": [16, 64],
    "proj_dim": [0, 8, 16, 32],
    "proj_style": ["direct", "linear", "mlp"],
    "injection": ["concat", "film"],
    "elev": ["with", "no"],
    "static": ["no", "with"],   # 'no' first as it's the main-matrix default
}


def marginal_knob_effects(
    df: pd.DataFrame,
    variable: str,
    metric: str = "mae",
    ...
    """Mean ± std of `metric` across configs sharing each (knob, value).

    Restricts to TESSERA-VAE entries only (so baselines don't muddy the
    averaging). Within a folder, every (knob, value) cell aggregates over
    all VAE configs that have that value, across all seeds.

    Args:
        df: enriched dataframe (must contain knob columns; call
            `enrich_with_knobs` first if not).
        variable: 't2m', 'wind', etc.
        metric: e.g. 'mae', 'rmse', 'nll'.
        target: filter to a specific target (e.g. 't2m', 'multitask').
            If None, includes all targets — usually you want this set.
        folders: restrict to a subset of folders. None = all.

    Returns:
        Long-form DataFrame with columns
        (folder, knob, knob_value, mean, std, n_configs, n_runs).
    """
    metric_col = f"{variable}_{metric}"
    if metric_col not in df.columns:
        return pd.DataFrame()

    sub = df[df["kind"] == "tessera_vae"].copy()
    sub = sub[sub[metric_col].notna()]
    if target is not None:
        sub = sub[sub["target"] == target]
    if folders is not None:
        sub = sub[sub["source_folder"].isin(folders)]
    if sub.empty:
        return pd.DataFrame()

    rows = []
    for folder in sorted(sub["source_folder"].unique()):
        folder_sub = sub[sub["source_folder"] == folder]
        for knob, values in KNOB_VALUES.items():
            for v in values:
                cell = folder_sub[folder_sub[knob] == v]
                if cell.empty:
                    continue
                rows.append({
                    "folder": folder,
                    "knob": knob,
                    "knob_value": v,
                    "mean": float(cell[metric_col].mean()),
                    "std": float(cell[metric_col].std() if len(cell) > 1 else 0.0),
                    "n_configs": cell["experiment"].nunique(),
                    "n_runs": len(cell),
                })
...
def rank_correlation_matrix(
    ...
    """Pairwise Spearman rank correlations of architecture rankings across folders.

    Within each folder, rank all TESSERA-VAE architectures by the metric
    (mean across seeds, lower = better rank 1). Then compute pairwise
    Spearman correlations across folders. High correlation (~0.9) means
    the architecture-choice signal is consistent across regions; low
    correlation means it's region-dependent (or noisy).

    Args:
        df: enriched dataframe.
        variable: 't2m', 'wind'.
        metric: 'mae' or 'rmse' typically.
        target: filter to a specific target (default: matches variable
            for single-task; pass 'multitask' explicitly if wanted).

    Returns:
        Square DataFrame indexed and columned by folder name.
    """
    metric_col = f"{variable}_{metric}"
    if metric_col not in df.columns:
        return pd.DataFrame()
    ...
    if target is not None:
        sub = sub[sub["target"] == target]
    elif variable in ("t2m", "wind"):
        # Default: single-task only when variable is t2m or wind.
        sub = sub[sub["target"] == variable]

    if sub.empty:
        return pd.DataFrame()

    # Average across seeds within each (folder, experiment) cell.
    grouped = (
        sub.groupby(["source_folder", "experiment"])[metric_col]
        .mean()
        .reset_index()
    )

    # Pivot to (folder × experiment) matrix, NaN where missing.
    pivot = grouped.pivot(index="experiment", columns="source_folder", values=metric_col)

    # Compute Spearman rank correlations between folder columns.
    return pivot.corr(method="spearman")


def mae_rmse_rank_consistency(
    df: pd.DataFrame,
    variable: str,
    target: str | None = None,
    distribution: str = "gaussian",
) -> pd.DataFrame:
    """Per-folder Spearman correlation between MAE rank and RMSE rank.

    Tells you whether picking architectures by MAE vs RMSE gives
    materially different shortlists. Most papers report RMSE so this
    is a sanity check that our MAE-driven shortlists generalise.

    ``distribution`` selects the MAE-equivalent and RMSE-equivalent
    columns: ``mae``/``rmse`` for Gaussian, ``mae_at_median``/
    ``rmse_at_mean`` for Weibull, ``wet_mae_at_median``/
    ``wet_rmse_at_mean`` for B-G. Defaults to Gaussian for legacy
    callers.

    Returns:
        DataFrame with columns (folder, spearman_corr, n_configs).
    """
    mae_col = f"{variable}_{_point_estimate_mae_column(distribution)}"
    rmse_col = f"{variable}_{_point_estimate_rmse_column(distribution)}"
    if mae_col not in df.columns or rmse_col not in df.columns:
        return pd.DataFrame()

    sub = df[df["kind"] == "tessera_vae"].copy()
    sub = sub[sub[mae_col].notna() & sub[rmse_col].notna()]
    if target is not None:
        sub = sub[sub["target"] == target]
    elif variable in ("t2m", "wind"):
        sub = sub[sub["target"] == variable]
    ...
    for folder in sorted(sub["source_folder"].unique()):
        folder_sub = sub[sub["source_folder"] == folder]
        # Average across seeds per experiment.
        per_exp = (
            folder_sub.groupby("experiment")[[mae_col, rmse_col]]
            .mean()
            .dropna()
        )
        if len(per_exp) < 2:
            continue
        corr = per_exp[mae_col].rank().corr(per_exp[rmse_col].rank(), method="spearman")
        rows.append({
            "folder": folder,
            "spearman_corr": float(corr),
            "n_configs": len(per_exp),
        })
    return pd.DataFrame(rows)

# -----------------------------------------------------------------------------
# Sensitivity-analysis helpers (Item 5, Angle A)
# -----------------------------------------------------------------------------
# Plotting helpers for the gradient-sensitivity output produced by
# tessera_downscaling.analysis.sensitivity. Lives here (rather than in
# a separate module) to keep the notebook's imports compact.

def _extract_sensitivity_quantity(results: dict, quantity: str) -> np.ndarray:
    """Pull a per-station array out of a sensitivity results dict.

    Quantities:
      - 'tessera_grad_norm'   per-station ‖∂μ/∂z‖
      - 'elev_grad_abs'        per-station |∂μ/∂elev| (per metre)
      - 'tessera_over_elev'    ratio of the above (NaN where elev grad ~ 0)
    """
    if quantity == "tessera_grad_norm":
        return results["station_tessera_grad_norm"]
    if quantity == "elev_grad_abs":
        return results["station_elev_grad_abs"]
    if quantity == "tessera_over_elev":
        with np.errstate(divide="ignore", invalid="ignore"):
            return (
                results["station_tessera_grad_norm"]
                / np.where(
                    results["station_elev_grad_abs"] > 1e-12,
                    results["station_elev_grad_abs"],
                    np.nan,
                )
            )
    raise ValueError(f"Unknown sensitivity quantity {quantity!r}")


def plot_per_station_saliency(
    results_by_label: dict,
    quantity: str = "tessera_grad_norm",
    figsize: tuple | None = None,
    cmap: str = "viridis",
    log_scale: bool = False,
    title_prefix: str = "",
):
    """Side-by-side scatter maps of stations coloured by saliency.

    Args:
        results_by_label: dict ``{friendly_label: results_dict}``, one
            panel per entry. Results dicts are produced by the
            sensitivity compute step (loaded via load_results).
        quantity: see ``_extract_sensitivity_quantity``.
        log_scale: useful when the distribution is heavy-tailed.
    """
    ...
    if figsize is None:
        figsize = (5.5 * max(n, 1), 4.5)
    fig, axes = plt.subplots(1, max(n, 1), figsize=figsize, squeeze=False)
    axes = axes[0]
    if n == 0:
        for ax in axes:
            ax.axis("off")
    ...
    # Global colour limits across panels for fair comparison.
    all_vals = []
    for r in results_by_label.values():
        v = _extract_sensitivity_quantity(r, quantity)
        all_vals.append(v[np.isfinite(v)])
    all_concat = np.concatenate(all_vals) if all_vals else np.array([])
    if all_concat.size == 0:
        for ax in axes:
            ax.axis("off")
        return fig
    vmin = float(np.nanpercentile(all_concat, 1))
    vmax = float(np.nanpercentile(all_concat, 99))
    if log_scale:
        positives = all_concat[all_concat > 0]
        if positives.size > 0:
            vmin = max(vmin, float(positives.min()))
        norm = LogNorm(vmin=vmin, vmax=vmax)
    else:
        norm = None

    for ax, (label, r) in zip(axes, results_by_label.items()):
        vals = _extract_sensitivity_quantity(r, quantity)
        finite = np.isfinite(vals)
        sc = ax.scatter(
            r["station_lons"][finite], r["station_lats"][finite],
            c=vals[finite], s=18, cmap=cmap,
            vmin=vmin if not log_scale else None,
            vmax=vmax if not log_scale else None,
            norm=norm, edgecolor="black", linewidth=0.2,
        )
        ax.set_xlabel("Longitude")
        ax.set_ylabel("Latitude")
        ax.set_title(f"{title_prefix}{label}", fontsize=10)
        ax.grid(alpha=0.3)
        plt.colorbar(sc, ax=ax, shrink=0.8, label=quantity)
    plt.tight_layout()
    return fig
...
def plot_channel_importance(
    ...
    """Bar chart of per-latent-dim TESSERA importance, stacked vertically.

    Args:
        normalise: divide each row's bars by their mean so cross-row
            *shape* is comparable. Without this, raw scales differ
            between concat / FiLM / different projection sizes and the
            visual comparison is misleading.
    """
    import matplotlib.pyplot as plt

    n = len(results_by_label)
    if figsize is None:
        figsize = (10, 1.8 * max(n, 1))
    fig, axes = plt.subplots(max(n, 1), 1, figsize=figsize, sharex=True, squeeze=False)
    axes = axes[:, 0]
    if n == 0:
        return fig

    for ax, (label, r) in zip(axes, results_by_label.items()):
        vals = r["channel_importance"].astype(np.float64)
        if normalise and vals.sum() > 0:
            vals = vals / vals.mean()
        latent_dim = r["meta"].get("precomputed_tessera_dim", len(vals))
        x = np.arange(len(vals))
        ax.bar(x, vals, width=0.85, color="#5b9bd5",
               edgecolor="black", linewidth=0.3)
        ax.axhline(1.0 if normalise else float(vals.mean()),
                   color="grey", linestyle=":", alpha=0.5)
        ax.set_title(label, fontsize=10)
        ax.set_ylabel("relative" if normalise else "raw |∂μ/∂z|")
        ax.grid(axis="y", alpha=0.3)
        proj_dim = r["meta"].get("vae_latents_proj_dim", 0) or 0
        if proj_dim and proj_dim != latent_dim:
            ax.axvline(proj_dim - 0.5, color="red", linestyle="--", alpha=0.3,
                       label=f"proj_dim={proj_dim}")
            ax.legend(fontsize=8)
    axes[-1].set_xlabel("latent dimension index")
    fig.suptitle("Per-channel TESSERA importance", fontsize=11)
    plt.tight_layout()
    return fig


def plot_tessera_vs_elev_distribution(
    results_by_label: dict,
    figsize: tuple | None = None,
):
    """Per-station histogram of log10(TESSERA grad norm / elev grad).

    Mass right of zero ⇒ TESSERA gradients dominate elevation gradients
    at most stations (model leans on TESSERA). Mass left of zero ⇒
    elevation dominates. Cross-region shape differences are the
    informative thing; absolute values aren't apples-to-apples since
    the two gradients have different units.
    """
    import matplotlib.pyplot as plt

    n = len(results_by_label)
    if figsize is None:
        figsize = (5.5 * max(n, 1), 4.0)
    fig, axes = plt.subplots(1, max(n, 1), figsize=figsize, squeeze=False)
    axes = axes[0]
    if n == 0:
        return fig
    ...
    for ax, (label, r) in zip(axes, results_by_label.items()):
        ratio = _extract_sensitivity_quantity(r, "tessera_over_elev")
        finite = np.isfinite(ratio) & (ratio > 0)
        if not finite.any():
            ax.axis("off")
            continue
        log_ratio = np.log10(ratio[finite])
        ax.hist(log_ratio, bins=40, color="#5b9bd5",
                edgecolor="black", linewidth=0.3)
        ax.axvline(0.0, color="red", linestyle="--", alpha=0.6,
                   label="ratio = 1 (TESSERA == elev)")
        ax.set_xlabel("log₁₀(‖∂μ/∂z‖ / |∂μ/∂elev|)")
...
def discover_sensitivity_results(directory: Path) -> dict:
    """Find all sensitivity ``.npz`` files in a directory, keyed by file stem."""
    directory = Path(directory)
    return {p.stem: p for p in sorted(directory.glob("*.npz"))}
